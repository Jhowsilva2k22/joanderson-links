...
import type { Link as LinkType, Profile } from "@/lib/supabase/types"
...
export default function LinksPage() {
  ...
  async function load() {
    ...
    const id: string = (profileData as Profile).id
    setProfileId(id)
  ...
  function openEdit(link: LinkType) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId) {
        await supabase
          .from("links")
          .update({ title: form.title.trim(), url, category: form.category })
          .eq("id", editingId)
      } else {
    ...
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>
            {links.length} {links.length === 1 ? "link" : "links"} cadastrado{links.length !== 1 ? "s" : ""}
          </p>
        </div>
        <button
          onClick={openCreate}
      ...
      {links.length === 0 ? (
        <div className="rounded-xl p-12 text-center" style={{ border: "1px dashed var(--border)" }}>
          ...
          <button onClick={openCreate} className="mt-3 text-sm font-medium underline underline-offset-2"
            style={{ color: "var(--foreground)" }}>Adicionar o primeiro</button>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {links.map((link) => (
            <div key={link.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
              style={{ background: "var(--card)", border: "1px solid var(--border)", opacity: link.active ? 1 : 0.5 }}>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button onClick={() => toggleActive(link)} className="p-1.5 rounded-lg hover:opacity-80"
                  style={{ color: "var(--muted-foreground)" }} title={link.active ? "Desativar" : "Ativar"}>
                  {link.active ? <Eye size={16} /> : <EyeOff size={16} />}
                </button>
                <button onClick={() => openEdit(link)} className="p-1.5 rounded-lg hover:opacity-80"
                  style={{ color: "var(--muted-foreground)" }} title="Editar">
                  <Pencil size={16} />
                </button>
                <button onClick={() => deleteLink(link.id)} className="p-1.5 rounded-lg hover:opacity-80"
                  style={{ color: "var(--destructive)" }} title="Excluir">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                ...
                <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
                  placeholder="ex: Instagram" className="w-full px-3 py-2 rounded-lg text-sm outline-none"
                  style={{ background: "var(--muted)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium" style={{ color: "var(--muted-foreground)" }}>URL</label>
                <input type="url" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })}
                  placeholder="https://" className="w-full px-3 py-2 rounded-lg text-sm outline-none"
                  style={{ background: "var(--muted)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
              </div>
              <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-3 mt-2">
                <button onClick={closeForm} className="flex-1 py-2.5 rounded-lg text-sm font-medium"
                  style={{ background: "var(--muted)", color: "var(--muted-foreground)", border: "1px solid var(--border)" }}>
                  Cancelar
                </button>
                <button onClick={handleSave} disabled={saving}
                  className="flex-1 py-2.5 rounded-lg text-sm font-semibold active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2"
                  style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>
